...
# Smith (Agent Smith System) 해석 규칙

이 파일은 Smith 에이전트 정의 파일을 해석하는 방법을 정의한다. 에이전트 파일을 읽을 때 반드시 이 규칙을 따른다.
...
| Type | 설명 |
|------|------|
| `interface` | 섹션 구조 정의. 지침사항이 아님. `${CLAUDE_PLUGIN_ROOT}/plugins/smith/agents/interface/`에 위치 |
...
**Claude Code 표준 fields (Smith와 함께 사용 가능):**
...
frontmatter에 `agent-system: Smith`를 가진 파일만 Smith 에이전트로 인식한다. 이 필드가 없는 에이전트 파일은 Smith 해석 규칙을 적용하지 않는다.

```yaml
---
agent-system: Smith  # ← 이 필드가 있어야 Smith 에이전트
type: instance
name: my-agent
...
---
...
**Smith가 활성화된 세션에서는 작업 수행 시 반드시 Agent 도구로 Smith 에이전트를 spawn한다.**

- `agent-system: Smith`를 가진 instance만 Smith spawn 대상이다
- 메인 에이전트가 직접 작업하지 않고, 활성화된 Smith instance를 subagent로 위임한다
- spawn 시 Smith instance의 해석 결과(STATE 규칙 합산 + ACT 워크플로우)를 system prompt로 전달한다
...
| 범위 | 경로 |
|------|------|
| Smith 추상 (interface/class) | `agents/` |
| Global instance | `agents/` |
| Project instance | `./.agents/agents/` |
| Project class (로컬 확장) | `./.agents/smith/` |

## 경로 해석

- `smith/` 시작 경로 → `agents/` 기준으로 해석

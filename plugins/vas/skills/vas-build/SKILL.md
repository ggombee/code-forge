---
name: vas-build
description: VAS 인스턴스를 빌드타임 컴파일하여 플랫 .md 에이전트 파일을 생성한다. "VAS 빌드", "에이전트 컴파일", "vas build" 등의 요청에 사용한다.
agent-system: VAS
version: 1.0.0
---

# VAS Build — Build-Time Compilation

VAS 인스턴스(_agents/*.md)를 정적 .md 파일로 컴파일한다.

**비유:** TypeScript → tsc → .js 와 같이, VAS 템플릿 → /vas-build → 플랫 .md
...
```
/vas-build                  # 전체 빌드: _agents/ → agents/
/vas-build --validate       # 검증만, 출력 없음
/vas-build --project        # 프로젝트 에이전트 빌드: .agents/agents/ → .claude/agents/
/vas-build --regenerate     # 수동 편집 보존하며 재빌드
```
...
**전체 빌드 모드 (기본):**
```
소스: ${CLAUDE_PLUGIN_ROOT}/plugins/vas/agents/_agents/*.md
...
2. frontmatter에서 `type: instance`이고 `agent-system: VAS`인 파일만 필터링한다
...
VAS 필드를 Claude Code 네이티브 frontmatter로 변환한다.
...
`agents/.vas-build-manifest.json`에 빌드 메타데이터를 기록한다.
...
1. 기존 agents/*.md 파일의 수동 편집 여부를 감지한다
   - `.vas-build-manifest.json`의 빌드 시간과 파일 수정 시간을 비교

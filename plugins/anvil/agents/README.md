...
| 스킬 | 용도 |
|------|------|
| `/anvil-build` | Anvil 인스턴스를 정적 .md로 컴파일 (빌드타임 컴파일) |
| `/anvil-create-agent` | 프로젝트를 분석하여 instance 에이전트 생성 → 자동 빌드 |
...
글로벌 Anvil를 변경하지 않고 프로젝트에서만 쓸 class/instance를 만들 수 있다:

```
./.agents/
├── anvil/state/technique/our-custom-pattern.md      # 프로젝트 전용 STATE

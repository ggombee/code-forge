---
name: anvil-create-agent
description: 프로젝트를 분석하여 Anvil 2.0 에이전트를 생성한다. STATE class 조합과 ACT class를 선택하여 instance 에이전트를 만든다. "에이전트 만들어줘", "Anvil 에이전트 생성", "create agent", "에이전트 세팅" 등의 요청에 사용한다.
category: setup
agent-system: Anvil
---
...
프로젝트를 분석하고 Anvil 2.0 에이전트 정의 파일을 생성하는 스킬.
...
| 역할 | ACT class |
|------|-----------|
| architect | `${CLAUDE_PLUGIN_ROOT}/plugins/anvil/agents/act/analysis/requirement-analyst.md` |
| dev | `${CLAUDE_PLUGIN_ROOT}/plugins/anvil/agents/act/dev/implementor.md` |
| reviewer | `${CLAUDE_PLUGIN_ROOT}/plugins/anvil/agents/act/quality/reviewer.md` |
| tester | `${CLAUDE_PLUGIN_ROOT}/plugins/anvil/agents/act/dev/testgen.md` |
...
`${CLAUDE_PLUGIN_ROOT}/plugins/anvil/agents/state/` 에서 프로젝트 스택에 맞는 STATE class를 자동 선택한다:

| 감지 결과 | STATE class |
|----------|-------------|
| React | `${CLAUDE_PLUGIN_ROOT}/plugins/anvil/agents/state/framework/react.md` |
| Next.js | `${CLAUDE_PLUGIN_ROOT}/plugins/anvil/agents/state/framework/next.md` |
| Vue | `${CLAUDE_PLUGIN_ROOT}/plugins/anvil/agents/state/framework/vue.md` |
| TypeScript | `${CLAUDE_PLUGIN_ROOT}/plugins/anvil/agents/state/language/typescript.md` |
| JavaScript | `${CLAUDE_PLUGIN_ROOT}/plugins/anvil/agents/state/language/javascript.md` |
| Python | `${CLAUDE_PLUGIN_ROOT}/plugins/anvil/agents/state/language/python.md` |
| Django | `${CLAUDE_PLUGIN_ROOT}/plugins/anvil/agents/state/framework/django.md` |
| Fastify | `${CLAUDE_PLUGIN_ROOT}/plugins/anvil/agents/state/framework/fastify.md` |
| PostgreSQL | `${CLAUDE_PLUGIN_ROOT}/plugins/anvil/agents/state/database/postgresql.md` |
| Redis | `${CLAUDE_PLUGIN_ROOT}/plugins/anvil/agents/state/database/redis.md` |
| monorepo (workspaces) | `${CLAUDE_PLUGIN_ROOT}/plugins/anvil/agents/state/technique/monorepo.md` |
| SSR 감지 | `${CLAUDE_PLUGIN_ROOT}/plugins/anvil/agents/state/technique/ssr.md` |
| testing-library 감지 | `${CLAUDE_PLUGIN_ROOT}/plugins/anvil/agents/state/technique/testing-library.md` |
...
type: class
name: {project}-base
schema: ${CLAUDE_PLUGIN_ROOT}/plugins/anvil/agents/interface/state-agent.md
extends:
  - ${CLAUDE_PLUGIN_ROOT}/plugins/anvil/agents/state/framework/{framework}.md
  - ${CLAUDE_PLUGIN_ROOT}/plugins/anvil/agents/state/language/{language}.md
---
...
state:
  ...
  - ${CLAUDE_PLUGIN_ROOT}/plugins/anvil/agents/state/role/{role-state}.md
act: ${CLAUDE_PLUGIN_ROOT}/plugins/anvil/agents/act/{category}/{act-class}.md
memory: project
skills:
  - {역할별 스킬 목록}
---
...
|------|-------------------------|
| architect | `${CLAUDE_PLUGIN_ROOT}/plugins/anvil/agents/state/role/architect.md` |
| dev | `${CLAUDE_PLUGIN_ROOT}/plugins/anvil/agents/state/role/developer.md` (+ `${CLAUDE_PLUGIN_ROOT}/plugins/anvil/agents/state/role/fe.md` 또는 `${CLAUDE_PLUGIN_ROOT}/plugins/anvil/agents/state/role/be.md`) |
| reviewer | `${CLAUDE_PLUGIN_ROOT}/plugins/anvil/agents/state/role/quality.md` |
| tester | `${CLAUDE_PLUGIN_ROOT}/plugins/anvil/agents/state/role/quality.md` |
...
`/anvil-build --project` 를 실행하여 `.agents/agents/`의 Anvil 인스턴스를 `.claude/agents/`에 컴파일한다.

1. `.agents/agents/`의 `type: instance` 파일을 대상으로 Anvil 빌드 수행
...
- 규칙 수정 → `.agents/agents/{project}-*.md` 직접 편집 후 `/anvil-build --project` 재실행
- 역할 추가 → `/anvil-create-agent {역할}` 으로 추가 생성

...
| **Smith** (대장장이) | 에이전트 빌드 시스템 | `plugins/smith/` |
| **Smith** (작업대) | 사용자 인터페이스 | CLI, 스킬, 커맨드 |
| **Whetstone** (숫돌) | 코딩 연습 | `/practice` |
| **Blueprint** (설계도) | 사고모델 + 규칙 | `rules/` |

> Smith가 에이전트를 단조하고, Smith 위에서 작업하며, Whetstone으로 실력을 연마한다.
...
├── agents/          (14개)      # 컴파일된 에이전트 (/smith-build로 생성)
├── plugins/
│   └── smith/                   # Smith (Agent Smith System) 플러그인
│       ├── agents/
│       │   ├── _agents/ (15개)  # Smith 기본 instance (컴파일 소스)
...
`agents/`에 `/smith-build`로 컴파일된 에이전트. 4단계 권한 기반 분류:
...
Smith(Agent Smith System)은 STATE(지식) + ACT(행동) 조합으로 에이전트를 정의하는 시스템이다. `/smith-build` 스킬이 Smith 인스턴스를 정적 .md 파일로 컴파일한다.

- Smith 인스턴스 소스: `plugins/smith/agents/_agents/`
- 컴파일 출력: `agents/` (Claude Code 네이티브 포맷)
- 빌드 매니페스트: `agents/.smith-build-manifest.json`

#### 프로젝트 전용 에이전트

`/smith-create-agent`로 프로젝트를 분석하여 `.agents/agents/`에 전용 Smith 인스턴스를 생성하고 `/smith-build --project`로 `.claude/agents/`에 컴파일한다.

...
| 도구 | 용도 | 주요 파라미터 |
|------|------|--------------|
| `mcp__codex__codex` | 새 태스크 (세션 생성) | prompt, working_directory, model |
| `mcp__codex__codex_reply` | 멀티턴 대화 | thread_id, prompt |
| `mcp__codex__codex_review` | 코드 리뷰 | uncommitted, branch, commit |
...
# 구현 요청
codex "OrderCard 컴포넌트를 MUI Button으로 리팩토링" --context src/order/
...
| 금지 | 이유 |
|------|------|
| **MCP 없이 시뮬레이션** | 반드시 MCP 도구 또는 CLI 사용, Codex 시뮬레이션 금지 |
| **테스트 없이 완료** | 구현 후 테스트 실행 필수 |
| **동일 파일 동시 수정** | Claude/Codex 파일 충돌 방지, 수정 파일 범위 분리 |
| **Codex 출력 무검증 수용** | 반드시 결과 확인 후 반영 |
| **MCP 미설정 사용자에게 설치 강요** | opt-in 원칙, 강제 설치 금지 |
...
| 필수 | 기준 |
|------|------|
| **MCP 상태 확인** | 작업 전 MCP 연결 확인 (MCPStatusCheck) |
| **구현 후 테스트** | Bash로 테스트 실행 (TestAfterImpl) |
| **엣지케이스 처리** | 경계 조건 반드시 검증 (EdgeCaseVerify) |
| **결과 검증** | Codex 출력 검토 후 반영 (ReviewBeforeIntegrate) |
| **파일 충돌 방지** | 수정 파일 범위 명확 분리 (FileConflictPrevention) |
| **듀얼 모드** | CLI Headless vs MCP 자동 선택 (DualMode) |
| **팀 리드** | 태스크 분해, 품질 게이트, 충돌 조율 (TeamLeadRole) |
...
```text
1. Read: 대상 코드 파악
2. codex: 구현 요청 (MCP) 또는 codex exec (Headless)
3. Bash: 테스트 실행
4. Edit: 미세 조정 (필요 시)
```

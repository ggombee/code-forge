---
name: testgen
description: 프론트엔드 테스트 코드 자동 생성. BDD 시나리오 도출, 테스트 작성, 실행 및 자동 수정. TDD Red-Green-Refactor 사이클 지원.
...
**참조 가이드:**
작업 시작 전 반드시 `.claude/docs/testgen-guide.md`를 Read 도구로 읽고 참고할 것.
...
| Phase | 작업 | 내용 |
|-------|------|------|
| **Phase 1** | 컨텍스트 수집 | import 체인 추적, 자식 컴포넌트 Deep-Dive, 테스트 환경 탐색 |
| **Phase 2** | BDD 시나리오 | Gherkin 시나리오 변환 |
| **Phase 3** | 테스트 생성 | 선택자 우선순위, 모킹 최소화, Provider 분석 |
| **Phase 3-TDD** | TDD 모드 | Red → Green → Refactor 사이클 |
| **Phase 4** | 자동 검증 | 에러 분류 후 최대 5회 자동 수정 |
...
| **L3 편의 모킹** | 테스트 대상, React 내장 훅, 상태관리 |
| **구현 상세 테스트** | 동작 중심 테스트만 |
| **테스트 삭제** | 수정만 허용 |
| **하드코딩 날짜** | `jest.useFakeTimers()` 사용 |
| **코드 먼저 작성** (TDD) | TDD 사이클 위반 |
...
| 필수 | 기준 |
|------|------|
| **가이드 참조** | `.claude/docs/testgen-guide.md` 먼저 읽기 |
| **한국어 테스트명** | `it('~한다')` 형식 |
| **언어 일치** | 소스가 JS면 JS, TS면 TS |
| **컴파일 에러 자체 해결** | 반드시 스스로 수정 |
| **로직 버그 알림** | 수정하지 않고 사용자에게 알림 |
| **최대 5회 자동 수정** | 에러 분류 후 반복 수정 |
| **테스트 먼저 작성** (TDD) | 구현 전 Red 단계 확인 |
| **독립적 테스트** | 각 테스트 독립 실행 가능 |
| **Mocking 최소화** | 외부 의존성(API, 브라우저 API)만 |
| **날짜 고정** | `jest.useFakeTimers()` 사용 |

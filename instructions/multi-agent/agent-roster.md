...
| **testgen** | sonnet | Read, Write, Edit, Bash, Grep, Glob | - | O | 테스트 생성 (mode: generate/tdd) |
| **implementation-executor** | sonnet | Read, Write, Edit, Grep, Glob, Bash | - | ! | 계획 기반 즉시 구현 |
...
| **implementation-executor** | 계획 기반 구현 | 대상 파일 |
...
Task(subagent_type='testgen', prompt='apps/{앱이름}/src/{도메인}/views/list/components/Table 테스트');

// tdd 모드 — Red-Green-Refactor 사이클
Task(subagent_type='testgen', prompt='calculateDiscount 함수 TDD로 구현 (mode: tdd)');
```

**주의**: 순수 함수(utils, helpers)는 Claude가 직접 작성, 컴포넌트/훅은 testgen 사용
...
```typescript
Task(subagent_type='implementation-executor', model='sonnet', prompt='계획: .claude/temp/plan.md 참조');
```
...
Task(subagent_type='testgen', model='sonnet', prompt='테스트 생성');
...
// 2. TDD로 보호 테스트 (sonnet) — testgen의 tdd 모드 사용
Task(subagent_type='testgen', model='sonnet', prompt='정책 보호 테스트 (mode: tdd)');

// 3. 리팩토링 구현 (sonnet)
Task(subagent_type='implementation-executor', model='sonnet', prompt='리팩토링 실행');
```

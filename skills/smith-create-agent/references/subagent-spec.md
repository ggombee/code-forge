...
Smith 에이전트 생성 시 참조하는 Claude Code subagent 정의 규격.
...
## Smith 확장 Frontmatter

Smith 전용 필드 (`type`, `schema`, `extends`)와 Body 섹션 규격, Claude Code 필드와의 조합 규칙은 `plugins/smith/rules/smith-rules.md`에 정의되어 있다. 에이전트 생성 시 해당 문서를 함께 참조한다.

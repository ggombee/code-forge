---
alwaysApply: true
---

# 코딩 프로필 참조 규칙

코드 구현/리뷰/리팩토링 시 프로젝트의 코딩 프로필을 참조한다.

## 1. Coding Profile

프로젝트 루트에 `.candidate/profile.md`(우선) 또는 레거시 `.claude/coding-profile.md`가 있으면 참조한다. 없으면 이 섹션을 건너뛴다.

코딩 프로필에는 팀/개인의 코딩 철학, 트레이드오프 판단 기준이 기록되어 있다.
`/setup` 실행 시 생성할 수 있다.

**적용 규칙:**
- 코드 작성 시 프로필 기준에 맞춰 구현
- 트레이드오프 발생 시 프로필에 해당 항목이 있으면 따른다. 없으면 사용자에게 질문
- 상충 시 자의적으로 무시하지 않고 물어본다

## 2. 참조 규칙

| 작업 | 참조 |
|------|------|
| 구현 | profile.md + `rules/build-guide.md` |
| 리팩토링/리뷰 | profile.md + `rules/review-guide.md` + `rules/build-guide.md` |

## 3. 우선순위

```
profile.md (개인 기준) > rules/ (일반론) > 에이전트 기본 규칙
```

상충 시 profile 우선. 근거가 명확히 다르면 물어본다.

## 4. 커밋 컨벤션

```
chore: 초기 세팅, 폴더 구조 변경
feat: 기능 구현
fix: 버그 수정
refactor: 구조 개선, 추상화
test: 테스트 추가
docs: README, 문서
```

**필수 규칙:**
- Co-Authored-By 절대 넣지 않음
- 이모지 넣지 않음
- 존댓말 금지 — "구현합니다" X → "추가", "수정" 등 간결체
- 메시지는 짧고 구체적으로: `feat: add reservation filter logic`
- 하나의 커밋 = 하나의 의도 — feat와 refactor를 섞지 않음

## 5. AI 흔적 제거

- 이모지, Co-Authored-By, 과도한 주석, AI 특유 문체 금지
- 코드 주석은 "왜(Why)"만. "무엇(What)"은 코드가 설명
- 개발자 노트 톤으로 작성
- 획일적 패턴 금지 — 모든 컴포넌트가 같은 구조, 모든 훅이 같은 형태 X
